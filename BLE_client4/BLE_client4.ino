...
void sendString(String value){
  ...
  for(int i=0;i<value.length();i=i+20){
    memcpy(buf,c_string+i,20);
    Serial.print((char*)buf);
    pRemoteCharacteristic->writeValue(buf, 20,true);
    sentSize += 20;
  }
  memcpy(buf,c_string+sentSize,value.length()-sentSize);
  Serial.print((char*)buf);
  pRemoteCharacteristic->writeValue(buf, value.length()-sentSize,true);
